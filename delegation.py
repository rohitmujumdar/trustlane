...
def validate_delegation(token: Optional[dict], key: bytes = DELEGATION_KEY) -> dict:
    """Verify signature + scope subset. Returns the {valid, scope_is_subset}
    ...
    """
    if not token:
        return {"valid": False, "scope_is_subset": False, "reason": "no token"}

    claimed_sig = token.get("signature", "")
    payload = {k: token[k] for k in ("agent_id", "principal", "scope", "parent_scope") if k in token}
    expected_sig = _sign(payload)
    sig_ok = hmac.compare_digest(claimed_sig, expected_sig)
    if not sig_ok:
        return {"valid": False, "scope_is_subset": False, "reason": "bad signature"}
